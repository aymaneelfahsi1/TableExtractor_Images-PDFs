...
# --- App Configuration ---
# Page config moved to main_app.py
...
class SingleShotTableProcessor:
    ...
    def extract_all_tables_in_one_shot(self, img: Image.Image, table_hint: int) -> List[Dict]:
        ...
        try:
            ...
            3.  **SEMANTIC DISAMBIGUATION (CRITICAL):** You must ensure all final column headers are unique and meaningful.
                -   NEVER use generic names like "Column header (TH)" or "Data cell (TD)" - these are HTML elements, not actual content.
                -   Extract the ACTUAL text content from each column header.
                -   If headers are truly empty, use descriptive names like "Column_1", "Column_2", etc.
                -   If simple headers are duplicated, rename them based on context (e.g., 'DATE_Start', 'DATE_End').
            ...
            # Post-process to fix duplicate column names
            all_tables_instructions = self._fix_duplicate_columns(all_tables_instructions)
    ...
    def _fix_duplicate_columns(self, tables_data: List[Dict]) -> List[Dict]:
        """Fix duplicate column names in table data."""
        if not isinstance(tables_data, list):
            return tables_data
            
        for table in tables_data:
            if not isinstance(table, dict) or 'column_structure' not in table:
                continue
                
            column_structure = table['column_structure']
            if not isinstance(column_structure, list):
                continue
                
            # Track column names to ensure uniqueness
            used_names = set()
            
            for i, col in enumerate(column_structure):
                if not isinstance(col, dict):
                    continue
                    
                original_name = col.get('name', '')
                
                # Skip generic HTML-like names and create meaningful ones
                if original_name in ['Column header (TH)', 'Data cell (TD)', '']:
                ...
                # Fix subheaders too
                subheaders = col.get('subheaders', [])
                if isinstance(subheaders, list):
                    for j, sub in enumerate(subheaders):
                        if isinstance(sub, dict):
                            sub_name = sub.get('name', '')
                            if sub_name in ['Column header (TH)', 'Data cell (TD)', '']:
                                sub['name'] = f'{new_name}_Sub_{j+1}'
                            elif sub_name in used_names:
                                counter = 1
                                while f"{sub_name}_{counter}" in used_names:
                                    counter += 1
                                sub['name'] = f"{sub_name}_{counter}"
                                used_names.add(f"{sub_name}_{counter}")
                            else:
                    ...
                    with st.expander(f"**{title}**", expanded=True):
                        ...
                        if flat_headers and (data_rows or summary_row):
                            try:
                                ...
                                # Check if data has more columns than headers
                                if preview_data and len(preview_data[0]) > len(flat_headers):
                                    # Adjust headers to match data columns
                                    num_data_cols = len(preview_data[0])
                                    adjusted_headers = flat_headers.copy()
                                    
                                    # Add missing headers
                                    for i in range(len(flat_headers), num_data_cols):
                                        adjusted_headers.append(f"Column_{i+1}")
                                    
                                    df = pd.DataFrame(preview_data, columns=adjusted_headers)
                                else:
